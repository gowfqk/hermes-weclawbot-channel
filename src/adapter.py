...
class WeClawBotAdapter(BasePlatformAdapter):
    ...
    async def _handle_inbound(self, message: dict[str, Any]) -> None:
        ...
        if not isinstance(text, str) or not text.strip():
            ...
            return
        # Bridge business semantics are single-user. The request ID is retained
        # as the response route until this run finishes, so interim tool updates
        # cannot consume the final reply address.
